  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="API类型" prop="apiType" width="120">
          <template #default="scope">
            {{ filterDict(scope.row.apiType, 'apiName') }}
          </template>
        </el-table-column>
        <el-table-column align="left" label="API状态" prop="status" width="120">
          <template #default="scope">
...
export default {
  ...
  methods: {
    ...
    async onDelete() {
      ...
      const res = await deleteApiTokenByIds({ ids })
      if (res.code === 0) {
        this.$message({
          type: 'success',
          message: '删除成功',
    ...
    async updateApiToken(row) {
      const res = await findApiToken({ ID: row.ID })
      this.type = 'update'
      if (res.code === 0) {
